...
class Settings:
    ...
    Attributes:
        ...
        llm_provider: ``auto`` / ``mock`` / ``openai`` / ``deepseek`` /
            ``alibaba`` / ``zhipu``。``auto`` 会按优先级在配置了 key 的
            provider 中选第一个；详见 ``llm/factory.py``。
        flask_debug: 是否开 Flask debug（仅本地开发；生产留 False）
        host: Web 服务绑定地址（默认仅本机 127.0.0.1）
        port: Web 服务端口

    Note:
        各家 LLM 的 API key 不在这里——``run_web.py`` 在启动期已经调过
        ``load_dotenv()``，需要 key 的 provider 直接 ``os.environ.get`` 取即可，
        没必要在 Settings 里再复述一遍。
    """
